...
class ShiftConv2d0(nn.Module):
    def __init__(self, inp_channels, out_channels):
        ...
        mask = nn.Parameter(torch.zeros(
            (self.out_channels, self.inp_channels, 3, 3)),
                            requires_grad=False)
...
class ShiftConv2d1(nn.Module):
    def __init__(self, inp_channels, out_channels):
        ...
        self.weight = nn.Parameter(torch.zeros(inp_channels, 1, 3, 3),
                                   requires_grad=False)
        self.n_div = 5
        g = inp_channels // self.n_div
...
class GMSA(nn.Module):
    def __init__(self,
        ...
        if self.calc_attn:
            self.split_chns = [
                channels * 2 // 3, channels * 2 // 3, channels * 2 // 3
            ]
...
class ELAB(nn.Module):
    def __init__(self,
        ...
        modules_lfe = {}
        modules_gmsa = {}
        modules_lfe['lfe_0'] = LFE(inp_channels=inp_channels,
                                   out_channels=out_channels,
                                   exp_ratio=exp_ratio)
        modules_gmsa['gmsa_0'] = GMSA(channels=inp_channels,
                                      shifts=shifts,
